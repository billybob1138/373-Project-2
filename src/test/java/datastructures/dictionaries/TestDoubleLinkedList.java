...
public class TestDoubleLinkedList extends BaseTest {
	/**
	 * Please call this method whenever you need to create a new
	 * DoubleLinkedList instance instead of calling the constructor directly.
	 * Our private tests rely on this method.
	 */
	...
	/**
	 * This method creates a simple list containing three elements to help
	 * minimize redundancy later in our tests.
	 *
	 * Please do not modify this method: our private tests will also use this
	 * method.
	 */
	...
	/**
	 * This test will check if a list contains exactly the same elements as the
	 * "expected" array. See the tests you were provided for example usage.
	 *
	...
	protected <T> void assertListMatches(T[] expected, IList<T> actual) {
		...
		for (int i = 0; i < expected.length; i++) {
			try {
				...
				String errorMessage = String.format("Got %s when getting item at index %d (expected '%s')",
	...
	public void testIndexOfAndContainsMany() {
		...
		// By setting the seed to some arbitrary but constant number, we
		// guarantee
		// this random number generator will produce the exact same sequence of
		// numbers
		// every time we run this test. This helps us keep our tests
		// deterministic, which
	...
	@Test(timeout = SECOND)
	public void testDeleteHappyCaseFromBack() {
		...
		assertTrue(list.delete(5) == 5);
		assertListMatches(expected, list);
	}

	@Test(timeout = SECOND)
	public void testDeleteHappyCaseFromFront() {
		Integer[] expected = new Integer[9];
		IList<Integer> list = this.makeInstance();
		for (int i = 0; i < 9; i++) {
			if (i >= 4) {
		...
		assertTrue(list.delete(4) == 4);
		assertListMatches(expected, list);
	}

	@Test(timeout = SECOND)
	public void testDeleteFromFrontCase() {
		IList<Integer> list = this.makeInstance();
		...
		list.delete(0); // deletes the last entry

		assertListMatches(testArray, list);
	}

	@Test(timeout = SECOND)
	public void testDeleteFromBackCase() {
		...
		list.delete(10); // deletes the last entry
	...
	public void testDeleteUpdatesSize() {
		...
		list.delete(5);
		expSize--;
		assertEquals("Delete method doesn't update size", list.size(), expSize);
	}
}
